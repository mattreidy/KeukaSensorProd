...
if [ -f "$LOG_DIR/duckdns_last.txt" ]; then
    if [ "$(stat -c%s "$LOG_DIR/duckdns_last.txt" 2>/dev/null || echo 0)" -gt 5242880 ]; then
        echo "Truncating large duckdns_last.txt (>5MB)..."
        # Keep last 1000 lines
        tail -n 1000 "$LOG_DIR/duckdns_last.txt" > "$LOG_DIR/duckdns_last.txt.tmp"
        mv "$LOG_DIR/duckdns_last.txt.tmp" "$LOG_DIR/duckdns_last.txt"
        chmod 600 "$LOG_DIR/duckdns_last.txt"
    fi
fi
...
if [ -f "$APP_ROOT/keuka/duckdns_last.txt" ]; then
    ...
    rm -f "$APP_ROOT/keuka/duckdns_last.txt"
fi

# 5. Rotate updater log if it gets too large (>1MB)
if [ -f "$LOG_DIR/updater.log" ]; then
    if [ "$(stat -c%s "$LOG_DIR/updater.log" 2>/dev/null || echo 0)" -gt 1048576 ]; then
        echo "Rotating large updater.log..."
        mv "$LOG_DIR/updater.log" "$LOG_DIR/updater.log.old"
        touch "$LOG_DIR/updater.log"
