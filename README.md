# Production Design: Raspberry Pi 3A+ Water Temp & Level Node (v1.4, chat export)
...
/opt/keuka-sensor/wifi-watchdog.sh
/etc/systemd/system/wifi-watchdog.service  ← (below)
```
...
1. Prep enclosure: drill lid 18 mm (JSN head), side 12 mm (PG7), 15–18 mm (PG9 power), 6.5–7 mm (RP-SMA bulkhead). Deburr, clean.
2. Mount hardware: Pi on standoffs; JSN controller PCB on stand-off panel; feed DS18B20 through PG7; mount RP-SMA bulkhead.
3. Wire: build 1 kΩ / 2 kΩ divider for ECHO; wire TRIG/ECHO/VCC/GND; add 4.7 kΩ pull-up for DS18B20.
4. Camera: place webcam lens to clear lid; foam shroud to reduce glare; tilt slightly to avoid seeing ultrasonic head.
5. Seal: silicone around transducer & glands; add desiccant.
6. Software: flash OS, enable 1-Wire, install deps, copy files, enable services.
7. Mount: strap the enclosure to a post 3–5 ft above water; drop DS18B20 to bottom with small stainless weight.
8. Verify: run through the testing checklist.
